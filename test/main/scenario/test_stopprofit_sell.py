...
from .profitloss_data import (FIELDS, stopprofit_sell, stopprofit_sell_fail, get_random_price_and_volume)
...
class TestStopProfitSell:
    ...
    @pytest.mark.parametrize(FIELDS, stopprofit_sell)
    def ytest_stopprofit_sell(self, market_id, price, entrust_type, trade_type,
        ...
        payload.price = float(new_price) + 0.04
        buy2_price = float(new_price) + 0.04
    ...
    @pytest.mark.parametrize(FIELDS, stopprofit_sell_fail)
    def test_stopprofit_sell_fail(self, market_id, price, entrust_type, trade_type,
                                  volume, trigger_price, auto_cancel_at, entrust_special_login):
        # login
        special_info = entrust_special_login([main_entrust_api, main_ac_api])
        payload = PostEntrustsRequest()
