#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/11/29 16:01
# @Author  : ZDK
# @Email    : zhengdengke@wanshare.com

# 新建文章——获取文章列表——获取文章详情——更新文章信息——获取文章列表——获取文章详情
import pytest
import random
...
class TestModelsCase5(object):

    @pytest.mark.order1
    def test_models_case5(self):
        # 后台登录
        ...
        while page <= total_page:
            ...
            for item in res.items:
                if item.title == up_payload1['title']:
                    ...
                    up_documents_id = item.id
            page += 1
        # 获取documents文章详情
        res = content_api.documents_id_get(id=up_documents_id)
