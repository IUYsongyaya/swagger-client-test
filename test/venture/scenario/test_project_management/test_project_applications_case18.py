#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/11/27 16:44
# @Author  : ZDK
# @Email    : zhengdengke@wanshare.com

# 申请项目——项目通过——查看项目列表——查看项目详情——修改coin信息——查看项目详情
...
class TestProjectCase18(object):

    @pytest.mark.order1
    def test_applications18(self):
        faker = Faker()
        ...
        short_name = "X" + str(random.randint(10000, 99999))
        ...
        payload1 = {
                      "projectInfo": {
                        ...
                        "telephone": "12874846",
                        "projectPosterKey": "url/image123455",
                        "description": "以太坊（英文Ethereum）是一个开源的有智能合约功能的公共区块链平台"
                                        }
                    }
        project_api.projects_id_put(id=audit_project_id, type="base", body=payload1)
        # 获取项目列表
        res = project_api.projects_get(page=1)
        audit_project_id = ''
        for item in res.items:
        ...
        assert res.coin_info.full_name == full_name
        assert res.coin_info.id
        assert res.project_info.cellphone == payload1['projectInfo']['cellphone']
        assert res.project_info.telephone == payload1['projectInfo']['telephone']
        assert res.project_info.description == payload1['projectInfo']['description']
