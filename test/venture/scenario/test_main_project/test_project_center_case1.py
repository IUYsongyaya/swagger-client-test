...
# 新增项目（项目方系统中获取项目列表与项目详情）——获取项目列表——获取项目详情
...
class TestCenterCase1(object):

    @pytest.mark.order1
    def test_project_center1(self):
        faker = Faker()
        ...
        short_name = "AW" + str(random.randint(100, 999))
        ...
        set_login_status(main_api, token)
        res = main_api.projects_get(page=1, sort_key="volume", limit=10)
        main_project_id = ''
        for item in res.items:
            main_project_id = item.project_id
        # 主平台获取项目详情
        res = main_api.projects_id_get(id=main_project_id)
        assert res.coin_info
        assert res.project_info
        assert res.sponsor_info
