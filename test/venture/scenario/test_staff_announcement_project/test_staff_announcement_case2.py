#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/11/21 18:23
# @Author  : ZDK
# @Email    : zhengdengke@wanshare.com

# 新建公告成功——(后台)获取项目方的所有项目列表——获取某一个项目公告列表——修改项目公告状态——获取某一个项目公告列表
...
class TestAnnouncementCase1(object):
    ...
    def test_announcement1(self):
        ...
        short_name = "AZ" + str(random.randint(100, 999))
        ...
        for item in res.items:
            ...
            announce_id = item.id
        # 后台修改公告状态
        staff_am_api.announcements_id_status_put(id=announce_id, status=False)
        # 后台获取项目公告列表
        staff_am_api = AnnouncementManagementApi()
        set_login_status(staff_am_api, token)
        res = staff_am_api.announcements_projects_list_project_id_get(project_id=audit_project_id)
