#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/11/20 18:16
# @Author  : ZDK
# @Email    : zhengdengke@wanshare.com

# 项目审核通过——>发送新闻资讯——>获取项目新闻资讯列表——>删除新闻资讯——>获取项目新闻资讯列表
...
class TestNewCase7(object):
    ...
    def test_new7(self):
        ...
        short_name = "E" + str(random.randint(10000, 99999))
        ...
        # 获取项目新闻资讯列表
        news_id = ''
        res = news_api.news_project_id_get(project_id=audit_project_id)
        assert res.meta.total_count == 1
        for item in res.items:
            assert item.title == payload1['title']
            assert item.link == payload1['link']
            assert item.source == payload1['source']
            assert item.id
            news_id = item.id
        # 删除项目新闻资讯
        news_api.news_id_new_delete(id=news_id)
        # 获取项目新闻资讯列表
        res = news_api.news_project_id_get(project_id=audit_project_id)
