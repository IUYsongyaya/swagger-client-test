#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/12/26 10:16
# @Author  : ZDK
# @Email    : zhengdengke@wanshare.com

# 前台新建项目并审核通过——查询项目方用户列表——获取项目列表——获取币种列表
...
class TestStaffVentureCase6(object):

    @pytest.mark.order1
    def test_staff_venture_case6(self):
        faker = Faker()
        ...
        short_name = "BT" + str(random.randint(1000, 9999))
        ...
        set_login_status(staff_project_api, token)
        res = staff_project_api.projects_account_id_get(id=venture_id)
        assert res.meta.total_count == 1
        for item in res.items:
            if item.project_name == project_name:
                assert item.id
        # 获取币种列表
        res = staff_project_api.projects_coins_init_get(init=True)
        item = res.pop()
        if item['shortName'] == short_name:
            assert item['shortName'] == short_name
