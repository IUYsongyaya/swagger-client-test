...
def test(setup):
    ...
    contact = _venture.query_contact(project_id=_project.project_id, sponsor="tenant", filter=dict(
        exchange_id=_exchange.exchange_id
    ))

    inviting_num = _venture.get_inviting_contact_num(_project.project_id)
    assert inviting_num, "inviting number is 0, no contact"

    # 项目方审批对接申请
    if contact:
        print("contact_id =============>", contact)
        _venture.contact_accept(contact_id=contact.contact_id, status="accepted")

    assert _tenant.check_contact(project_id=_project.project_id, exchange_id=_exchange.exchange_id)
